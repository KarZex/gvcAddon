...
with open("_summon.txt","r") as f:
    ...
    for i in l:
        ...
        if mode == "r":
            ...
            block_json["minecraft:block"]["events"]["gvcv5:spawn"]["run_command"]["command"].append("execute if block ~ ~-1 ~ gvcv5:spawn_addon_ga run fill ~~~ ~~-1~ air")
            block_json["minecraft:block"]["events"]["gvcv5:spawn"]["run_command"]["command"].append("execute if block ~ ~-1 ~ gvcv5:spawn_addon_ca run fill ~~~ ~~-1~ air")
            block_json["minecraft:block"]["events"]["gvcv5:spawn"]["run_command"]["command"].append("execute unless block ~ ~-1 ~ gvcv5:spawn_addon_ga unless block ~ ~-1 ~ gvcv5:spawn_addon_ca run fill ~~~ ~~~ air")
        else:
            block_json["minecraft:block"]["events"]["gvcv5:spawn"]["run_command"]["command"].append("kill @e[type={},r=1]".format(e_id))
