...
for row in csv_reader:
    ...
    if( row_count >= 1 and row[4] != "" ):
        ...
        with open("behavior_packs/GVCAddonV5(2)/functions/structure/{}.mcfunction".format(structure_id),"w") as f:
            ...
            f.write("execute unless block ~{0}~-{1}~{2} air if block ~{0}~8~{2} air run structure load {3} ~~-{1}~\n".format(structure_loadx,structure_loady,structure_loadz,structure_id))
            if( structure_loadx > 64 ): f.write("execute unless block ~{0}~-{1}~{2} air if block ~{0}~8~{2} air run structure load {3}_x64 ~64~-{1}~\n".format(structure_loadx,structure_loady,structure_loadz,structure_id))
            if( structure_loadz > 64 ): f.write("execute unless block ~{0}~-{1}~{2} air if block ~{0}~8~{2} air run structure load {3}_z64 ~~-{1}~64\n".format(structure_loadx,structure_loady,structure_loadz,structure_id))
            if( structure_loadx > 64 and structure_loadz > 64 ): f.write("execute unless block ~{0}~-{1}~{2} air if block ~{0}~8~{2} air run structure load {3}_x64z64 ~64~-{1}~64\n".format(structure_loadx,structure_loady,structure_loadz,structure_id))
            f.write("execute if block ~{0}~-{1}~{2} air run tickingarea remove {3}\n".format(structure_loadx,structure_loady/2,structure_loadz,structure_id))
            f.write("execute unless block ~{0}~8~{2} air run tickingarea remove {3}\n".format(structure_loadx,structure_loady/2,structure_loadz,structure_id))
